    ...
    def transition_Root_idle(self, event) :
        ...
        if len(enableds) > 0 :
            ...
            if enabled == 1 :
                self.exit_Root_idle()
                self.button.pressed()
                self.controller.outputEvent(Event("button-pressed", port="", parameters = []))
                self.enter_Root_idle()
            elif enabled == 2 :
                self.exit_Root_idle()
                self.button.show_tooltip()
                self.enter_Root_idle()
            elif enabled == 3 :
                self.exit_Root_idle()
                self.button.hide_tooltip()
                self.enter_Root_idle()
            catched = True
...
class Controller(GameLoopControllerBase):
    def __init__(self, button, keep_running = True):
        super(Controller, self).__init__(ObjectManager(self), keep_running)
        self.addInputPort("input")
        self.object_manager.createInstance("Main", [button])
